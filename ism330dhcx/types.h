...
typedef u8_t bool8_t;                                           // 8-bit boolean type
#if defined(__STDC_VERSION__) && __STDC_VERSION__ >= 199901L    // C99 or later supports _Bool
typedef _Bool bool_t;                                           // Native boolean type
#else
typedef bool8_t bool_t;                                         // Boolean type (fallback to 8-bit)
#endif /* Boolean type */
...
/// @union adr_u8
/// @brief 8-bit address
typedef union adr_u8
{
    u8_t full;    // 8-bit register full address

    /// @struct byte8
    /// @brief  8-bit register address byte
    struct byte8 {
...
typedef union adr_u16
{
    u16_t full;   // 16-bit register full address

    /// @struct byte16
    /// @brief 16-bit register address bytes
    struct byte16 {
...
typedef union adr_u32
{
    u32_t full;   // 32-bit register full address

    /// @struct byte32
    /// @brief 32-bit register address bytes
    struct byte32 {
...
/// @union adr_u64
/// @brief 64-bit address
typedef union adr_u64
{
    u64_t full;   // 64-bit register full address

    /// @struct byte64
    /// @brief 64-bit register address bytes
    struct byte64 {
